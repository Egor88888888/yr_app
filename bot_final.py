...
async def web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ADMIN_CHAT_ID:
        logger.warning("v6.0 | ADMIN_CHAT_ID не найден. Уведомление не будет отправлено.")
        return
    ...
    try:
        ...
        logger.error(f"v6.0 | КРИТИЧЕСКАЯ ОШИБКА при отправке уведомления администратору: {e}")


def main() -> None:
    """Основная функция для запуска бота."""
    if not TOKEN:
        logger.critical("ОШИБКА ЗАПУСКА: Переменная YOUR_BOT_TOKEN не найдена!")
        return

    if not MY_PUBLIC_URL:
        logger.critical("ОШИБКА ЗАПУСКА: Переменная MY_RAILWAY_PUBLIC_URL не найдена! Убедитесь, что она добавлена в 'Variables'.")
        return
