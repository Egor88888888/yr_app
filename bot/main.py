...
async def _fallback_autopost(context: ContextTypes.DEFAULT_TYPE):
    """Enhanced fallback автопостинг с качественными структурированными постами"""

    try:
        # Используем улучшенный PostGenerator для качественных постов
        ...
        log.info("📝 Generating enhanced educational post...")
        post_text = await enhanced_generator.generate_post()

        log.info(f"✅ Generated enhanced post: {post_text[:100]}...")

    except Exception as e:
        log.error(f"❌ Enhanced post generation failed: {e}")

        # Резервный fallback к старой системе
        advanced_topics = [
            "Как правильно обжаловать штраф ГИБДД: пошаговая инструкция",
            "Защита трудовых прав: алгоритм действий при нарушениях",
            "Жилищные споры с УК: практическое руководство",
            "Налоговые вычеты 2024: полный алгоритм получения",
            "Банкротство физлица: когда подавать и как избежать ошибок",
            "Семейные споры: как защитить интересы детей в суде",
            "Электронные госуслуги: решение проблем при отказах",
            "Защита прав потребителей: пошаговый алгоритм возврата",
            "Наследственные споры: как правильно вступить в наследство",
            "Миграционное право: получение гражданства РФ по шагам"
        ]

        topic = random.choice(advanced_topics)

        enhanced_prompt = f"""Ты - ведущий юрист-эксперт, создающий максимально полезные практические гиды.

ЗАДАЧА: Создать структурированный пост на тему "{topic}"

ОБЯЗАТЕЛЬНАЯ СТРУКТУРА:
🎯 ЗАГОЛОВОК
📋 ПОШАГОВЫЙ АЛГОРИТМ (4-6 конкретных шагов с номерами)
⚠️ ВОЗМОЖНЫЕ ПРОБЛЕМЫ (3-4 типичные ситуации)
🔗 ПОЛЕЗНЫЕ РЕСУРСЫ (реальные ссылки на госсайты)
📞 ПРИЗЫВ К ДЕЙСТВИЮ

ТРЕБОВАНИЯ:
✅ Конкретные действия с указанием сроков
✅ Реальные ссылки: gosuslugi.ru, nalog.gov.ru, rosreestr.gov.ru и др.
✅ Предупреждения о подводных камнях
✅ Простые формулировки для обычных людей
✅ Эмодзи для лучшего восприятия
✅ Объем: 700-900 символов

СТИЛЬ: Экспертный, но доступный. Максимально практично."""
        ...
        messages = [{
            ...
            "role": "user",
            "content": f"Создай детальный практический гид: {topic}"
        }]

        post_text = await generate_ai_response(messages, model="openai/gpt-4o", max_tokens=1200)
