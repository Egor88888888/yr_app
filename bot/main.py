...
async def _fallback_autopost(context: ContextTypes.DEFAULT_TYPE):
    """ПРОФЕССИОНАЛЬНЫЙ fallback автопостинг с идеальной структурой"""

    try:
        # Используем улучшенный PostGenerator
        from bot.services.content_intelligence.post_generator import PostGenerator

        enhanced_generator = PostGenerator()

        # Генерируем качественный образовательный пост
        log.info("📝 Generating professional educational post...")
        post_text = await enhanced_generator.generate_post()

        log.info(f"✅ Generated professional post: {len(post_text)} chars")

    except Exception as e:
        log.error(f"❌ Enhanced post generation failed: {e}")

        # ЭКСТРЕННЫЙ РЕЗЕРВ: Создаем идеальный пост вручную
        high_quality_topics = [
            {
                'title': 'Как подать жалобу в Роспотребнадзор',
                'algorithm': '''1. Подготовьте документы (чеки, договоры, фото)
2. Заполните заявление на сайте rospotrebnadzor.ru
3. Укажите суть нарушения и ваши требования
4. Приложите доказательства к заявлению
5. Дождитесь ответа в течение 30 дней''',
                'problems': '''• Отсутствие чеков и документов
• Пропуск 2-летнего срока подачи
• Неточное описание нарушения
• Отказ продавца в добровольном решении''',
                'resources': '''🛡️ Роспотребнадзор: rospotrebnadzor.ru
🏛️ Госуслуги: gosuslugi.ru
📞 Горячая линия: 8-800-100-00-04'''
            },
            {
                'title': 'Защита трудовых прав работника',
                'algorithm': '''1. Изучите трудовой договор и ТК РФ
2. Зафиксируйте нарушение письменно
3. Подайте заявление работодателю
4. Обратитесь в трудовую инспекцию
5. При неудаче - подавайте в суд''',
                'problems': '''• Отсутствие письменных доказательств
• Пропуск сроков обращения
• Давление со стороны работодателя
• Незнание своих прав''',
                'resources': '''💼 Трудовая инспекция: git.com
🏛️ Госуслуги: gosuslugi.ru
📖 ТК РФ: consultant.ru'''
            },
            {
                'title': 'Обжалование штрафов ГИБДД',
                'algorithm': '''1. Получите постановление о штрафе
2. Изучите основания для обжалования
3. Подготовьте доказательства невиновности
4. Подайте жалобу в течение 10 дней
5. Дождитесь решения в течение 10 дней''',
                'problems': '''• Пропуск 10-дневного срока
• Отсутствие доказательств невиновности
• Неправильное оформление жалобы
• Ошибки в указании данных''',
                'resources': '''🚔 ГИБДД: gibdd.ru
🏛️ Госуслуги: gosuslugi.ru
📋 Суды общей юрисдикции: sudrf.ru'''
        ...
        import random
        topic = random.choice(high_quality_topics)

        # Создаем ИДЕАЛЬНЫЙ пост
...
Получите персональную консультацию: /start"""
        ...
        log.info(f"🛡️ Using emergency fallback post: {len(post_text)} chars")
