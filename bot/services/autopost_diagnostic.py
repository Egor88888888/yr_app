...
async def create_test_autopost(bot):
    """
    Создание тестового автопоста для проверки системы
    """
    try:
        import os
        ...
        if not channel_id:
            return "❌ **ОШИБКА:** TARGET_CHANNEL_ID не настроен"

        # Создаем тестовое содержимое
        test_content = """🔧 **ТЕСТОВЫЙ ПОСТ АВТОПОСТИНГА**

⚖️ Проверка системы автоматической публикации
...
🎯 **РЕЗУЛЬТАТ:** Автопостинг успешно настроен!

📱 Нужна консультация? Используйте кнопку ниже!"""

        # Инициализируем SMM Integration
        smm = SMMIntegration(bot)

        # Запускаем систему если не запущена
        if not smm.is_running:
            await smm.start_smm_system()

        # Создаем и публикуем тестовый пост
        result = await smm.create_and_publish_post(
        ...
        if result.get('success'):
            return f"""✅ **ТЕСТОВЫЙ ПОСТ СОЗДАН!**

📨 **Пост ID:** {result.get('post_id', 'Unknown')}
🆔 **Message ID:** {result.get('message_id', 'Unknown')}
📋 **Канал:** {channel_id}
⏰ **Время:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

🎯 **РЕЗУЛЬТАТ:** Система автопостинга работает корректно!"""
        else:
...
3. Проверьте правильность CHANNEL_ID"""
        ...
        return f"❌ **ОШИБКА ТЕСТИРОВАНИЯ:** {e}"
